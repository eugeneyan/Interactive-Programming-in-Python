...
# helper to draw each rock/missle
...
# group_collide helper to check for collision and remove rocks/missiles
def group_collide(group, other_object):
    for item in set(group):
        if item.collide(other_object):
            group.remove(item)
            explosion = Sprite(item.pos, [0, 0], 0, 0, explosion_image,
                                       explosion_info, explosion_sound)
            explosion_group.add(explosion)
            return True

# group_group_collide helper to remove collided missiles and rocks        
...
def rock_spawner():
    ...
    # multiple velocity by 1 + percentage of score to make rocks faster as score increases
    rock_vel = [(random.random() * .6 - .3) * (1 + score * 0.05), (random.random() * .6 - .3) * (1 + score * 0.05)]
    rock_avel = random.random() * .2 - .1
    a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    if len(rock_group) < 12 and started and dist(rock_pos, my_ship.pos) > 100:
